# EmotionAI Pro - 融合版情感智能插件 v4.1.3
...
### v4.1.3（负好感阶段亲密度固定为 0）

- 关系变差、好感度掉到负数后，亲密度立刻清零并固定为 0：这段关系的目标是「修复」，不是「继续积累亲密度」；
- 亲密度只在好感度为正时才累积；好感度回到 0 以上后从 0 重新开始，不会被历史亲密度「白送」；
- 这条规则高于一切亲密度机制——每轮情感变化、过渡期加成、首次深度交流 / 连续多日互动两类里程碑、管理员手动设置，在负好感期间一律不生效；
- `/设置亲密` 在用户处于负好感时会拒绝执行并说明原因；`/设置好感` 调到负数时会提示亲密度已同步清零；
- 状态面板对负好感用户的「好感度 100% | 亲密度 0%」权重展示保持不变，语义与新的锁定规则一致。

### v4.1.2（亲密度门槛不再压低已到位的老阶段）

- 亲密度门槛现在只作用于「迈向下一阶段」的过渡：在还没有亲密度门槛的年代就已经到位的阶段不会被压低，面板保持当前关系阶段；等分数涨到下一阶段时，门槛照常生效，未达标依旧会卡住并提示还差多少点，达标后正常完成过渡；
- 关系变差、分数真的垮下去时该降级还是降级，不会因为上面这条变成永久停留；
- 补上阶段显示名 → 内部阶段的反查，用户改过阶段名后，存档里的旧名字也能正确识别。
