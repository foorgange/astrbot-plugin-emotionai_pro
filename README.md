# EmotionAI Pro - 融合版情感智能插件 v4.0.23
...
v4.0.22 引入的统一亲密度门槛有一个结构性缺陷：它只有一个固定数值，而**第二、三次
过渡根本拦不到东西**。以承诺期为例——该阶段权重是「好感度 0.3 / 亲密度 0.7」，
复合评分要冲到 80，本身就要求亲密度达到 72 以上（好感度 100 时），40 的统
一门槛在那一刻形同虚设。真正被门槛约束住的只有第一次过渡。

本次把门槛改成**按目标阶段分别设置**，每次过渡各按各的档位生效。

| 配置项 | 默认值 | 说明 |
|---|---|---|
| `stage_intimacy_gates` | `{"DEEPENING": 20, "COMMITMENT": 40, "SYMBIOSIS": 60}` | 分阶段门槛：key 是目标阶段（第二/三/四阶段），值是占最大亲密度的百分比 |

- 三次过渡的门槛各自独立，随「最大亲密度」配置一起缩放：上限 200 时即
  40 / 80 / 120，上限改了自动跟着变，无任何写死的绝对数值；
- 某个阶段在表里删掉不填，该段自动回退用上方的统一门槛
  `transition_intimacy_pct`（老配置不受影响，行为与 v4.0.22 一致）；
- 某一档填 `0` 只关闭那一段的门槛，不影响其它档；
- 过渡被卡住时，面板 / `/好感度` / 进阶建议显示的「还差多少点」按**被挡住
  那个阶段**的档位计算，与过渡加成用的是同一套口径；
- 新增 12 项测试，含「第二次过渡不再形同虚设」的回归用例与「同一状态在
  统一门槛下会放行」的对照用例；全量 362 项通过。
